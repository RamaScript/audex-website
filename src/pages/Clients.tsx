...
export function ClientsPage() {
  useDocumentMeta('Our Clients | Audex', 'Trusted by industry leaders in logistics and discrete manufacturing.')
    ...
    <div className="bg-white text-deepsea">
      ...
      <section className="bg-[#F7F8FC] border-b border-deepsea/5 pt-32 pb-24 text-center">
        <Container>
          ...
          >
            Our <span className="font-bold text-[var(--color-blue)]">Clients</span>
          </motion.h1>
          ...
          >
            We empower global industry leaders with robust, intelligent logistics solutions, driving operational excellence across multiple sectors.
          </motion.p>
        </Container>
      </section>
            ...
            {categories.map((category, idx) => (
              ...
              >
                ...
                <div className="bg-deepsea text-white py-4 px-6 rounded-t-[2px] shadow-sm text-center">
                  <h3 className="font-display text-[18px] font-bold tracking-[0.02em]">{category.title}</h3>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-8 bg-[#f0f9f6] border border-deepsea/5 rounded-b-[2px] h-full shadow-inner">
                  {category.clients.map((logo, index) => (
                    <motion.div
                      ...
                      className="flex items-center justify-center p-6 bg-white rounded shadow-sm border border-deepsea/5 transition-all duration-300 hover:shadow-md"
                    >
                      <img
                        src={logo}
                        alt="Client logo"
                        className="max-h-16 w-auto object-contain mix-blend-multiply opacity-90 transition-opacity hover:opacity-100"
                      />
                    </motion.div>
                  ))}
                </div>
